 ...
 * @param {boolean} [useTarGz] - If true, creates a tar.gz archive instead of a zip.
 ...
 */
export async function zipFiles(files, useTarGz, progressCallback) {
  const path = await import('path');
  const fs = await import('fs');
  const archiver = (await import('archiver')).default;

  return new Promise((resolve, reject) => {
    logAndProgress('Creating in-memory archive...', progressCallback);
    const chunks = [];
