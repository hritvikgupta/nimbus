...
export const create = async (): Promise<GcloudExecutable> => {
  ...
  return {
    ...
    lint: async (command: string): Promise<ParsedGcloudLintResult> => {
      ...
      // Else, success.
      // Newer gcloud emits `command_string_no_args`; older (e.g. 495) emits `command_string`.
      const parsed = lintCommand.command_string_no_args ?? lintCommand.command_string ?? '';
      return {
        success: true,
        // Remove gcloud prefix since we added it in during the invocation, above.
        parsedCommand: parsed.slice('gcloud '.length),
      };
    },
  };
};
...
// Field name differs by gcloud version: newer = command_string_no_args, older = command_string.
const LintCommandSchema = z.object({
  command_string_no_args: z.string().optional(),
